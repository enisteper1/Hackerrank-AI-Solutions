        ...
        if row > 0:
            if self.grid[row - 1][col] != '%' \
               and (row - 1, col) not in self.visited:
                # calculate cost of next position
                gn = abs(self.pac_pos_r - (row - 1)) + abs(self.pac_pos_c - col)
                hn = abs(self.food_pos_r - (row - 1)) + abs(self.food_pos_c - col)
                # add next position with cost to temporary steps list
                temp_steps.append((row -1, col, gn + hn))
        ...
        if col > 0: 
            if self.grid[row][col - 1] != '%' \
               and (row, col - 1) not in self.visited:
                # calculate cost of next position
                gn = abs(self.pac_pos_r - (row)) + abs(self.pac_pos_c - (col - 1))
                hn = abs(self.food_pos_r - (row)) + abs(self.food_pos_c - (col - 1))
                # add next position with cost to temporary steps list
                temp_steps.append((row, col - 1, gn + hn))
